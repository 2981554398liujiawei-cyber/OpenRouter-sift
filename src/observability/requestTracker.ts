  ...
  private async enrich(id: string, generationId: string, restartRecovery = false): Promise<void> {
    try {
      ...
      this.store.update(id, {
        ...
        cachedPromptTokens: data.cachedPromptTokens ?? this.store.get(id)?.cachedPromptTokens ?? null,
        cacheWriteTokens: data.cacheWriteTokens ?? this.store.get(id)?.cacheWriteTokens ?? null,
        cacheDiscountUsd: data.cacheDiscountUsd ?? this.store.get(id)?.cacheDiscountUsd ?? null,
