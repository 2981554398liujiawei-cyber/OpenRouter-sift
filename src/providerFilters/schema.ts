...
export const providerFilterConditionSchema = z.object({
  ...
  if ((condition.operator === "in" || condition.operator === "notIn") && (!Array.isArray(condition.value) || condition.value.length === 0)) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ["value"], message: "Membership filters require a non-empty array" });
  }
