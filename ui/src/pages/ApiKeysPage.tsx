...
export function ApiKeysPage({ desired, setNotice, setError }: { desired: DesiredModel[]; setNotice: (value: string) => void; setError: (value: string) => void }) {
  ...
  const [keys, setKeys] = useState<AccessKey[]>([]); const [sessionSecrets, setSessionSecrets] = useState<Record<string, string>>({}); const [editing, setEditing] = useState<AccessKey | null>(null); const [routingKey, setRoutingKey] = useState<AccessKey | null>(null); const [showForm, setShowForm] = useState(false); const [secret, setSecret] = useState<AccessKeySecret | null>(null);
  ...
  const save = async (name: string, allowedModels: string[], enabled: boolean): Promise<void> => {
    ...
    setSessionSecrets((current) => ({ ...current, [created.id]: created.secret }));
    setSecret(created); setShowForm(false); setEditing(null); setNotice(t("access.keyCreated"));
    try { await load(false); } catch { setError(t("access.listRefreshFailed")); }
  };
  const copyKey = async (item: AccessKey) => {
    const value = sessionSecrets[item.id];
    if (!value) { setError(t("access.copyUnavailable")); return; }
  ...
  const remove = async (item: AccessKey) => { if (!window.confirm(t("access.confirmDelete", { name: item.name }))) return; try { await api.deleteAccessKey(item.id); setSessionSecrets((current) => { const next = { ...current }; delete next[item.id]; return next; }); await load(); setNotice(t("access.keyDeleted")); } catch (err) { setError((err as Error).message); } };
