...
const modeCopy: Record<OverrideMode, string> = {
  inherit: "Use the model's own routing policy for this key.",
  allowlist: "Only the selected providers may serve this key.",
  blocklist: "The selected providers are excluded for this key.",
};
...
export function AccessKeyRouting({ keyId, allowedModels, onClose, onSaved, setError, setNotice }: { keyId: string; allowedModels: string[]; onClose: () => void; onSaved?: () => void; setError: (value: string) => void; setNotice: (value: string) => void }) {
  ...
  const save = async () => { try { await api.saveAccessKeyRouting(keyId, draft); setNotice("Provider access saved."); onSaved?.(); await load(); } catch (err) { setError((err as Error).message); } };
  const reset = async () => { try { await api.resetAccessKeyRouting(keyId, modelId); setDraft(empty(modelId)); setPreview(null); setNotice("Provider access reset to inherit."); await load(); } catch (err) { setError((err as Error).message); } };
  ...
  return <div className="modal-backdrop"><section className="modal panel key-routing" role="dialog" aria-label="Provider access">
    <div className="panel-title"><div><p className="eyebrow">API Key</p><h2>Provider Access</h2><p>Further restrict which providers this API key may use for this model.</p></div><button className="text-button" onClick={onClose}>Close</button></div>
    {loading ? <p className="muted">Loading provider access…</p> : allowedModels.length === 0 ? <p className="muted">No models are assigned to this key.</p> : <>
      ...
      <div className="access-summary">
        <span>Desired Model currently allows: <strong>{modelAllows} {modelAllows === 1 ? "provider" : "providers"}</strong></span>
        <span>This API key allows: <strong>{keyAllows} {keyAllows === 1 ? "provider" : "providers"}</strong></span>
      </div>
      <fieldset className="mode-select mode-cards"><legend>Mode</legend>{(["inherit", "allowlist", "blocklist"] as OverrideMode[]).map((mode) => <label key={mode} className="mode-card"><input type="radio" name="key-routing-mode" checked={draft.mode === mode} onChange={() => setDraft({ ...draft, mode })} /><span><strong>{mode === "inherit" ? "Inherit" : mode === "allowlist" ? "Allow selected" : "Block selected"}</strong><small>{modeCopy[mode]}</small></span></label>)}</fieldset>
      {draft.mode !== "inherit" && <><div className="provider-picker"><h3>Providers</h3>{providers.length === 0 ? <p className="muted">Provider catalog unavailable.</p> : providers.map((provider) => <label className={`provider-choice ${provider.available === false ? "provider-unavailable" : ""}`} key={provider.id}><input type="checkbox" disabled={provider.available === false} checked={draft.providers.includes(provider.id)} onChange={() => toggleProvider(provider.id)} /><span><strong>{provider.name}</strong><small className="mono">{provider.id} · {provider.available === false ? (provider.status ?? "Excluded by Desired Model filters") : "Available"}</small></span></label>)}</div>{draft.mode === "allowlist" && <div className="order"><h3>Priority</h3>{(draft.providerOrder.length ? draft.providerOrder : draft.providers).map((id, index, order) => <div key={id}><span>{index + 1}. {providers.find((provider) => provider.id === id)?.name ?? id}</span><span><button aria-label={`Move ${id} up`} disabled={index === 0} onClick={() => move(id, -1)}>↑</button><button aria-label={`Move ${id} down`} disabled={index === order.length - 1} onClick={() => move(id, 1)}>↓</button></span></div>)}</div>}<details className="advanced-block"><summary>Advanced</summary><label className="switch"><input type="checkbox" checked={draft.allowFallbacks} onChange={(event) => setDraft({ ...draft, allowFallbacks: event.target.checked })} />Allow fallback providers</label><small>Fallback stays inside the providers allowed above.</small></details></>}
      ...
      <div className="actions"><button className="button secondary" onClick={() => void reset()}>Reset</button><button className="button" onClick={() => void save()}>Save</button></div>
    </>}
  </section></div>;
}
