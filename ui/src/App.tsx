...
function ModelDetail({ modelId, onBack, onSaved, setNotice, setError }: { modelId: string; onBack: () => void; onSaved: () => void; setNotice: (value: string) => void; setError: (value: string) => void }) {
  ...
  const [tab, setTab] = useState<"overview" | "capabilities" | "providers">("overview");
  ...
  const openTab = (next: "overview" | "capabilities" | "providers") => { setTab(next); if (next === "providers" && !providersLoaded && !providersLoading) void loadProviders(); };
  ...
  return <section className="page"><button className="back" onClick={onBack}>← Models</button><div className="page-heading detail-heading"><div><p className="eyebrow">Model control</p><h1>{model.name || model.id}</h1><code>{model.creator ?? "Unknown creator"} · {model.id}</code></div>{tab === "providers" && providersLoaded && <button className="button secondary" onClick={() => void loadProviders(true)}>Refresh Providers</button>}</div>
    <div className="detail-tabs" role="tablist" aria-label="Model details">{(["overview", "capabilities", "providers"] as const).map((item) => <button key={item} role="tab" aria-selected={tab === item} className={tab === item ? "tab-active" : ""} onClick={() => openTab(item)}>{item[0].toUpperCase() + item.slice(1)}</button>)}</div>
    {tab === "overview" && <><div className="stats"><Stat label="Context" value={model.contextLength ? model.contextLength.toLocaleString() : "—"} /><Stat label="Input" value={perMillion(model.pricing, "prompt")} /><Stat label="Output" value={perMillion(model.pricing, "completion")} /></div><section className="panel"><h2>Overview</h2><p>{model.description || "Model metadata from the local OpenRouter catalog."}</p><dl className="detail-grid"><dt>Creator</dt><dd>{model.creator ?? "—"}</dd><dt>Model ID</dt><dd><code>{model.id}</code></dd><dt>Created</dt><dd>{model.created ? new Date(model.created * 1000).toLocaleDateString() : "—"}</dd><dt>Max completion</dt><dd>{model.maxCompletionTokens?.toLocaleString() ?? "—"}</dd></dl></section></>}
