...
describe("provider filter API and proxy", () => it("previews and hard-limits managed inference by endpoint tag", async () => {
  const dir = mkdtempSync(join(tmpdir(), "sift-filter-")); const forwarded: any[] = [];
  try {
    globalThis.fetch = vi.fn(async (url, init) => {
      if (String(url).includes("/endpoints")) return new Response(JSON.stringify({ data: { endpoints: [
    ...
    const preview = await (await nativeFetch(`${base}/api/desired-models/demo%2Fmodel/filter/preview`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ conditions: filter.conditions }) })).json() as any;
